...
export function AdminSecuritySetup({ enabled }: { enabled: boolean }) {
  ...
  if (complete) {
    return (
      <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950 dark:text-emerald-100">
      ...
      <form className="max-w-xl space-y-5" onSubmit={enable}>
        <div>
          ...
          <input
            ...
            className="mt-2 min-h-12 w-full rounded-xl border bg-white px-4 dark:bg-slate-950"
        ...
        <button className="inline-flex min-h-12 items-center gap-2 rounded-xl bg-[#007FA8] px-5 font-bold text-white disabled:opacity-60" disabled={loading} type="submit">
          <ShieldCheckIcon aria-hidden size={20} />
          {loading ? "Preparando" : "Ativar segundo fator"}
        </button>
        {error ? <p role="alert" className="text-sm font-bold text-red-700 dark:text-red-300">{error}</p> : null}
      </form>
    );
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <section className="rounded-2xl border bg-slate-50 p-5 dark:bg-slate-950">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <a className="inline-flex min-h-11 items-center rounded-xl bg-[#007FA8] px-4 text-sm font-bold text-white" href={enrollment.totpURI}>
      ...
      {enrollment.backupCodes.length ? (
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-950 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
      ...
      <form className="space-y-4" onSubmit={verify}>
        <div>
          ...
          <input
            ...
            className="mt-2 min-h-12 w-full max-w-xs rounded-xl border bg-white px-4 font-mono text-lg tracking-[0.12em] dark:bg-slate-950"
            id="enrollment-code"
            inputMode="numeric"
            maxLength={6}
            name="code"
        ...
        <button className="inline-flex min-h-12 items-center rounded-xl bg-[#007FA8] px-5 font-bold text-white disabled:opacity-60" disabled={loading} type="submit">
