...
export default async function CartPage() {
  if (isLealBrindeApiConfigured()) {
    const cart = await lealBrindeApi<{
      items: Array<{ id: string; productName: string; productSlug: string; imageUrl: string | null; sku: string | null; options: Record<string, string>; quantity: number; unit: "UNIT" | "METER"; unitPriceCents: number; totalCents: number }>;
      subtotalCents: number;
      paymentMethods: Array<"PIX" | "CREDIT_CARD">;
    }>("/v1/carts/current", { forwardCookies: true }).catch(() => ({ items: [], subtotalCents: 0, paymentMethods: ["PIX"] as Array<"PIX" | "CREDIT_CARD"> }));
    return <main id="conteudo" className="bg-surface-subtle py-10 sm:py-16"><div className="mx-auto max-w-shell px-4 sm:px-6 lg:px-8"><header className="mb-10 max-w-3xl"><p className="text-xs font-bold uppercase tracking-[0.16em] text-accent">Sua seleção</p><h1 className="mt-3 text-5xl font-black tracking-[-0.05em] sm:text-6xl">Carrinho</h1><p className="mt-4 text-base leading-7 text-muted">Revise produtos e quantidades. Frete e pagamento são confirmados na próxima etapa.</p></header><CartView initial={cart} /></div></main>;
  }
